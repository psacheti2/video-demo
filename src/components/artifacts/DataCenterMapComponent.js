...
const DataCenterMapComponent = ({ onLayersReady, onSaveMap, savedMaps = [], setSavedArtifacts, title,
    onBack, isPreview = false, center = [-3.7038, 40.4168], onSendMessage, originConversationId, artifactsPanelWidth }) => {
    const [layerZIndexes, setLayerZIndexes] = useState({
    dataCenters: 30
});
const [topInteractiveLayer, setTopInteractiveLayer] = useState('dataCenters');
...
const [tableTitles, setTableTitles] = useState(['Data Centers']);
...
const highlightMultipleFeatures = (rowIndices) => {
    ...
    if (rowIndices.length > 1) {
        ...
        rowIndices.forEach(rowIndex => {
            ...
            if (currentTableIndex === 0) {
                const dataCenter = dataCentersRef.current.find(dc => dc.featureId === row['Feature ID']);
                if (dataCenter && dataCenter.marker) {
                    bounds.extend(dataCenter.marker.getLatLng());
                }
            }
        });
    ...
    if (Array.isArray(rowIndices)) {
        ...
        switch (currentTableIndex) {
            case 0: // Data Centers
                highlightDataCenter(row);
                break;
        }
    }
};
const highlightDataCenter = (row, isMultiSelect = false) => {
    if (!row || !map) return;

    const featureId = row['Feature ID'];

    if (featureId) {
        const matchedDataCenter = dataCentersRef.current.find(dc => dc.featureId === featureId);

        if (matchedDataCenter) {
            highlightMarker(matchedDataCenter.marker, isMultiSelect);
            return;
        }
    }
    ...
    map.on('click', function (e) {
        ...
        if (currentTopLayer === 'dataCenters') {
            ...
            if (nearestMarker) {
                console.log('Nearest data center found:', nearestMarker.featureId);
        ...
        if (typeof html2canvas === 'undefined') {
            ...
            script.onload = () => {
                html2canvas(mapNode, {
                    ...
                    link.href = imgData;
link.download = `madrid_datacenter_map_${new Date().toISOString().slice(0, 10)}.jpg`;
...
link.download = `madrid_datacenter_map_${new Date().toISOString().slice(0, 10)}.jpg`;
    ...
    // Only handle data centers layer
    if (map.dataCenterLayer) {
        if (activeLayers.dataCenters) {
            map.addLayer(map.dataCenterLayer);
            map.dataCenterLayer.setZIndex(layerZIndexes.dataCenters);

            dataCentersRef.current.forEach(({ marker }) => {
                marker.options.interactive = (topInteractiveLayer === 'dataCenters');
            ...
            map.removeLayer(map.dataCenterLayer);
        }
    }
        ...
        Object.entries(downloadSelections).forEach(([key, { filename, format }]) => {
            ...
            if (key === 'map') {
                ...
                if (format === '.pdf') {
                    ...
                    if (map && dataCentersRef.current.length > 0) {
    dataCentersRef.current.forEach(({ marker }) => {
        const position = marker.getLatLng();
        const popupContent = marker.getPopup()?.getContent() || '';

        let name = "Data Center";
        const nameMatch = popupContent.match(/Name: ([^<]+)/);
        if (nameMatch && nameMatch[1]) {
            name = nameMatch[1].trim();
        }

        mapData.dataCenters.push({
            lat: position.lat,
            lng: position.lng,
            name: name,
            type: 'Data Center'
...
useEffect(() => {
    ...
    Object.entries(layerZIndexes).forEach(([layerId, zIndex]) => {
        ...
        let layer;
        if (layerId === 'dataCenters') layer = map.dataCenterLayer;

        if (layer) {
            layer.setZIndex(zIndex);

            // Also update individual markers for data centers
            if (layerId === 'dataCenters') {
                dataCentersRef.current.forEach(({ marker }) => {
                    if (marker._icon) {
                        marker._icon.style.zIndex = zIndex + 10;
                    }
                });
            }
        }
...
useEffect(() => {
    ...
    // Data centers layer - make only these interactive if they're the top layer
    if (topInteractiveLayer === 'dataCenters') {
        dataCentersRef.current.forEach(({ marker }) => {
            ...
            marker.on('click', function (e) {
                console.log('Data center clicked:', marker.featureId);
      ...
      if (layer.layerId) {
        ...
        setDrawnLayersOrder(prev => {
          ...
          return newOrder;
            ...
            const [dataCenterResult] = await Promise.all([
    fetchDataCenters(),
]);

if (dataCenterResult.dataCenterData) {
    const dataCenterTable = convertGeoJSONToTable(dataCenterResult.dataCenterData, 'dataCenters');
    setTableData([dataCenterTable]);
}
...
dataCenterResult.dataCenterLayer.setZIndex(layerZIndexes.dataCenters);
dataCenterResult.dataCenterLayer.addTo(leafletMap);

leafletMap.dataCenterLayer = dataCenterResult.dataCenterLayer;
...
useEffect(() => {
    ...
    <a href="#" className="text-blue-600 underline hover:text-blue-800">
        Madrid Data Center Registry
    </a>
</div>
<div>
    <a href="#" className="text-blue-600 underline hover:text-blue-800">
        European Colocation Database
    </a>
</div>
<div>
    <a href="#" className="text-blue-600 underline hover:text-blue-800">
        Infrastructure Site Analysis
    </a>
</div>
                        </div>
                    </div>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    const artifact = {
                                        ...
                                        title: name,
                                        type: 'map',
                                        component: 'DataCenterMapComponent',
props: {
...
export default DataCenterMapComponent;