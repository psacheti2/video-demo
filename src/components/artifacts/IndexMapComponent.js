...
const fetchFloodZones = async () => {
  try {
    const res = await fetch('/data/madridfloodzones-data.geojson');
    ...
    const riskLevels = [
      { 
        ...
        weight: 0.15,
        description: 'Extreme flood risk - immediate evacuation recommended'
      },
      { 
        level: 'High', 
        color: '#E74C3C', // Red
        weight: 0.25,
        description: 'High flood risk - avoid area during flood warnings'
      },
      { 
        level: 'Moderate', 
        color: '#F39C12', // Orange
      ...
      { 
        ...
        color: '#F1C40F', // Yellow
        weight: 0.20,
        description: 'Low flood risk - general awareness advised'
      },
      { 
        level: 'Minimal', 
        color: '#27AE60', // Green
        weight: 0.05,
        description: 'Minimal flood risk - safe under normal conditions'
      }
    ];
    ...
    if (floodData.features && floodData.features.length > 0) {
      ...
      floodData.features.forEach((feature, index) => {
        ...
        feature.properties['Feature ID'] = feature.properties['Feature ID'] || `flood-${index + 1}`;
      });
      ...
      floodData.features.forEach((feature, index) => {
        if (feature.geometry && feature.geometry.type === 'Polygon') {
          ...
          const riskData = calculateRiskLevel(feature, index, floodData.features.length);
          ...
          popupContent += `<h4 style="margin: 0 0 10px 0; color: ${riskData.color};">🌊 Flood Risk Zone</h4>`;
          popupContent += `<strong>Risk Level:</strong> <span style="color: ${riskData.color}; font-weight: bold;">${riskData.level}</span><br>`;
          popupContent += `<strong>Feature ID:</strong> ${featureId}<br>`;
          popupContent += `<strong>Zone Type:</strong> ${feature.properties.fclass || 'River Buffer'}<br>`;
          popupContent += `<small style="color: #666; font-style: italic;">${riskData.description}</small>`;
          popupContent += `</div>`;
...
const fetchZoningBoundaries = async () => {
  try {
    const res = await fetch('/data/madridzoning-data.geojson');
    ...
    if (zoningData.features && zoningData.features.length > 0) {
      ...
      industrialFeatures.forEach((feature) => {
        if (feature.geometry && feature.geometry.type === 'Polygon') {
          ...
          zoningBoundariesRef.current.push({ polygon, featureId });
    ...
    return { zoningLayer: L.layerGroup(), zoningData: { features: [] } };
  }
};

const generateComplianceIndicators = () => {
  const complianceLayer = L.layerGroup();
  
  uploadedLocationsRef.current.forEach(({ marker, featureId }, index) => {
    const position = marker.getLatLng();
    
    // Use featureId to determine consistent status
    const statusPattern = ['compliant', 'compliant', 'warning', 'warning', 'violation'];
    const featureIdNum = parseInt(featureId.replace(/\D/g, '')) || 0; // Extract number from featureId
    const complianceStatus = statusPattern[featureIdNum % statusPattern.length];
    const colors = {
      compliant: '#27AE60',
      warning: '#F39C12', 
      violation: '#E74C3C'
    ...
    complianceMarker.featureId = `compliance-${index + 1}`;

    let popupContent = '<strong>Compliance Status</strong>';
    popupContent += `<br>Status: ${statusText[complianceStatus]}`;
    popupContent += `<br>Last Check: ${new Date().toLocaleDateString()}`;
    
    if (complianceStatus === 'warning') {
      popupContent += `<br>Issue: Environmental permit renewal required`;
    } else if (complianceStatus === 'violation') {
      popupContent += `<br>Issue: Exceeds noise regulations`;
    }

    complianceMarker.bindPopup(popupContent, { className: 'custom-popup' });

    complianceLayer.addLayer(complianceMarker);
  });
...
const fetchEnvironmentalRisk = async () => {
  try {
    ...
    const riskHotspots = [
      { lat: 40.430, lng: -3.692, intensity: 1.0, name: "Industrial Zone North" },
      { lat: 40.408, lng: -3.715, intensity: 0.9, name: "Chemical Plant Area" },
      { lat: 40.395, lng: -3.698, intensity: 0.8, name: "Waste Treatment Facility" },
      { lat: 40.445, lng: -3.680, intensity: 0.7, name: "Heavy Traffic Corridor" },
      { lat: 40.385, lng: -3.725, intensity: 0.6, name: "Airport Vicinity" }
    ];
    
    // Generate dense point cloud around hotspots
    riskHotspots.forEach(hotspot => {
      // Add main hotspot point
      heatmapData.push([hotspot.lat, hotspot.lng, hotspot.intensity]);
      
      // Add surrounding points with decreasing intensity
      for (let i = 0; i < 30; i++) {
        const distance = Math.random() * 0.01; // Random distance up to ~1km
        const angle = Math.random() * 2 * Math.PI;
        const lat = hotspot.lat + distance * Math.cos(angle);
        const lng = hotspot.lng + distance * Math.sin(angle);
        const intensity = hotspot.intensity * (0.3 + Math.random() * 0.4); // 30-70% of main intensity
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const intensity = Math.random() * 0.3; // Low background risk
    ...
    const heatmapLayer = L.heatLayer(heatmapData, {
      ...
      gradient: {
        ...
        0.2: '#F1C40F',  // Yellow - Low-moderate risk
        0.4: '#F39C12',  // Orange - Moderate risk
        0.6: '#E67E22',  // Dark orange - High-moderate risk
        0.8: '#E74C3C',  // Red - High risk
        1.0: '#8B0000'   // Dark red - Very high risk
      }
    });
    
    // Store hotspot info for popups/interaction
    const riskData = {
      features: riskHotspots.map((hotspot, index) => ({
        properties: {
          'Feature ID': `env-risk-${index + 1}`,
          'Risk Level': hotspot.intensity > 0.8 ? 'Very High' : 
                       hotspot.intensity > 0.6 ? 'High' : 
                       hotspot.intensity > 0.4 ? 'Moderate' : 'Low',
          'Location': hotspot.name,
          'Risk Score': `${(hotspot.intensity * 100).toFixed(1)}%`
        },
        geometry: {
          type: 'Point',
    ...
    return { riskLayer: heatmapLayer, riskData };
  } catch (error) {
    console.error("Error generating environmental risk heatmap:", error);
    return { riskLayer: L.layerGroup(), riskData: { features: [] } };
  }
};
...
const createRankingTableData = () => {
  ...
  const locationScores = uploadedLocationsRef.current.map(({ featureId }, index) => {
    const compliance = getComplianceStatus(index);
    let score;
...
const locationName = getLocationName(scoreData.originalIndex);    
    ...
    const recommendation = rankIndex === 0 ? 'Proceed with acquisition' :
                          ...
                          rankIndex === 2 ? 'Viable backup location' :
                          rankIndex === 3 ? 'Consider with reservations' :
                          'Not recommended';
    
    return {
      'Feature ID': scoreData.featureId,
  ...
  return {
    headers: ['Feature ID', 'Rank', 'Location', 'Weighted Score', 'Infrastructure Score', 'Regulatory Score', 'Environmental Score', 'Economic Score', 'Recommendation'],
    rows
  };
};
...
powerlinesRef.current.forEach(({ polyline }) => {
  ...
  // Handle flood zones layer
if (map.floodLayer) {
  if (activeLayers.floodZones) {
    map.addLayer(map.floodLayer);
    map.floodLayer.setZIndex(layerZIndexes.floodZones);
  } else {
    map.removeLayer(map.floodLayer);
  }
}
...
if (map.zoningLayer) {
  if (activeLayers.zoningBoundaries) {
    ...
    map.zoningLayer.setZIndex(layerZIndexes.zoningBoundaries);
  } else {
    map.removeLayer(map.zoningLayer);
  }
}

// Handle compliance indicators layer
if (map.complianceLayer) {
  if (activeLayers.complianceIndicators) {
    map.addLayer(map.complianceLayer);
    map.complianceLayer.setZIndex(layerZIndexes.complianceIndicators);
  } else {
    map.removeLayer(map.complianceLayer);
  }
}

if (map.environmentalRiskLayer) {
  if (activeLayers.environmentalRisk) {
    map.addLayer(map.environmentalRiskLayer);
  } else {
    map.removeLayer(map.environmentalRiskLayer);
  }
}
if (map.weightedScoringLayer) {
  if (activeLayers.weightedScoring) {
    map.addLayer(map.weightedScoringLayer);
    map.weightedScoringLayer.setZIndex(layerZIndexes.weightedScoring);
  } else {
    map.removeLayer(map.weightedScoringLayer);
  }
}
...
if (map && map.environmentalRiskLayer && map.environmentalRiskLayer.setOptions) {
  map.environmentalRiskLayer.setOptions({
    gradient: {
      0.0: layerColors.environmentalRisk || '#27AE60',
      0.2: '#F1C40F',
      0.4: '#F39C12', 
      0.6: '#E67E22',
      0.8: '#E74C3C',
